...
const eval = async (ctx, many = false, global = false) => {
    ...
    if (global) {
        ...
        return ctx.send(ctx, `Your query consisting of ${ctx.boldName(ctx.fmtNum(ctx.globalCards.length))} cards is worth ${ctx.boldName(ctx.fmtNum(evals))} ${ctx.symbols.tomato}`)
    }
